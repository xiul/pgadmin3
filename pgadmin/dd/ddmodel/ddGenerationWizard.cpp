...
{
	wxString strChoicesSimple[1] = {wxT("Create Table")};
	wxString strChoicesAlter[4] = {wxT("Alter Table"), wxT("Drop it, then Create Table"), wxT("Create Table [Conflict]"), wxT("No Action")};
...
: wxWizardPage(parent)
{
	...
	if(wparent && wparent->getConnection())
		...
		schemasNames.Add(_("Not Schema Selected -- Disable Alter Table instead of Create Statments function"));
...
void SelGenSchemaPage::OnWizardPageChanging(wxWizardEvent& event)
{
	...
	else if(event.GetDirection())
	{
		...
		else
		{
			...
			wparent->schemaName=_("Non existing schema");
		}
...
void SelGenSchemaPage::refreshSchemas(pgConn *connection)
{
	...
	schemasHM.clear();
	schemasNames.Clear();
	schemasNames.Add(wxT("Not Schema Selected -- Disable Alter Table instead of Create Statments function"));
...
: wxWizardPage(parent)
{
	...
	//Add a message
	message = new wxStaticText(this, wxID_STATIC, _("Please review desired selection at each table to be generated:"), wxDefaultPosition, wxDefaultSize, wxALIGN_LEFT);
	topSizer->Add(message);
	topSizer->AddSpacer(10);
...
void ReportGridPage::populateGrid()
{
	...
	for(row=0;row<max;row++)
	{
		...
		if(table!=NULL && table->getBelongsToSchema())
		{
			...
			reportGrid->SetCellValue( row, 1, wxT("Alter Table"));
	...
	for(row=0;row<max;row++)
	{
		...
		if(table!=NULL && !table->getBelongsToSchema())
		{
			...
			reportGrid->SetCellValue( row, 1, wxT("Create Table"));
			useRow++;
		}
		else if(table==NULL)
		{
			wxMessageBox(_("Unexpected error, metadata of table to be generated not found at database designer model"), _("Error importing at generation process"),  wxICON_ERROR);
			return;
		}
...
void ReportGridPage::OnWizardPageChanging(wxWizardEvent& event)
{
	if(!event.GetDirection())
	{
		...
		int answer = wxMessageBox(_("Returning to select schema page all selections will be lost. Do you want to continue?"), _("Confirm"), wxYES_NO | wxCANCEL, this);
		if (answer != wxYES)
		{
			event.Veto();
		}
