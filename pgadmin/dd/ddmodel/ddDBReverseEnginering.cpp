		...
		if (foreignKeys && foreignKeys->NumRows() >0)
		{
			while (!foreignKeys->Eof())
			{
				...
				if(sourceSchema.IsSameAs(destSchema,false))
				{
					...
					if(!destTabFigure)
					{
						wxMessageBox(_("Table used as destination of relationship not found"), _("Error importing relationship"),  wxICON_ERROR);
						return;
					}
					...
					if(!sourceTabFigure)
					{
						wxMessageBox(_("Table used as source of relationship not found"), _("Error importing relationship"),  wxICON_ERROR);
						return;
					}
...
wxArrayString ddDBReverseEngineering::getTables() //pgConn *connection, OID oidSchema)
{
	...
	query = wxT("SELECT oid, relname, relkind\n")
	        wxT("  FROM pg_class\n")
	        wxT(" WHERE relkind IN ('r','v','x') AND relnamespace = ") + NumToStr(OIDSelectedSchema) + wxT(";");
	...
	else if(event.GetDirection())
	{
		...
		if( itemsCount > 0)
		{
			...
			do{
				...
				if(wparent->getDesign()->getTable(m_selTables->GetString(item))!=NULL)
				{
