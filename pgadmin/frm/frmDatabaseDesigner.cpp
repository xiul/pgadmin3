...
BEGIN_EVENT_TABLE(frmDatabaseDesigner, pgFrame)
	...
	EVT_BUTTON(CTL_CHGSCHEMA, frmDatabaseDesigner::OnSelectSchema)
	...
	: pgFrame(NULL, _title)
{
	...
	diagramMenu->Append(MNU_NEWDIAGRAM, _("&New model diagram"), _("Create a new diagram for open database design"));
	diagramMenu->Append(MNU_DELDIAGRAM, _("&Delete selected model diagram..."), _("Delete selected diagram tab of database design"));
	diagramMenu->Append(MNU_RENDIAGRAM, _("&Rename selected model diagram..."), _("Rename selected diagram tab of database design"));
	...
	toolBar->AddTool(MNU_NEWDIAGRAM, _("New Diagram"), *ddnewdiagram_png_bmp, _("Create a new diagram for open database design [Testing purpose]"), wxITEM_NORMAL);
	...
	toolBar->AddTool(MNU_GENERATEDIAGRAM, _("Generate Selected Diagram"), *ddgendiagram_png_bmp, _("Generate SQL for the current diagram"), wxITEM_NORMAL);
	toolBar->AddSeparator();
	...
	selectedSchema = new wxTextCtrl(connectionPanel, wxID_ANY, _("..."), wxDefaultPosition, wxDefaultSize, wxTE_CENTRE);
	selectedSchema->SetEditable(false);
	selectedSchema->SetBackgroundColour(*wxLIGHT_GREY);
	selSchemaButton = new wxButton( connectionPanel, CTL_CHGSCHEMA, wxT("&Select Schema"), wxDefaultPosition, wxDefaultSize, 0 );
	
	connectionSizer->Add(cbConnection, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 1);
	connectionSizer->AddSpacer(5);
	connectionSizer->Add(selectedSchema, 0, wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL, 1);
	connectionSizer->AddSpacer(5);
	connectionSizer->Add(selSchemaButton, 0, wxALIGN_CENTER_VERTICAL | wxALL, 1);
	connectionSizer->AddSpacer(5);
...
// Add the panes
	...
	manager.AddPane(connectionPanel, wxAuiPaneInfo().Name(wxT("databaseBar")).Caption(_("Connection bar")).ToolbarPane().Top().LeftDockable(false).RightDockable(false));
...
void frmDatabaseDesigner::setModelChanged(bool value) 
{
	...
	if(previousChanged != changed)
	{
		...
		wxToolBarToolBase *itemToolbar = toolBar->FindById(MNU_SAVEMODEL);
...
void frmDatabaseDesigner::OnAddTable(wxCommandEvent &event)
{
	...
	{
		wxMessageBox(_("Warning about adding table without a diagram"), _("Please create a model diagram first"), wxICON_EXCLAMATION);
	}
}
...
void frmDatabaseDesigner::OnDeleteTable(wxCommandEvent &event)
{
	...
	{
		wxMessageBox(_("Warning about deleting table without a diagram"),_("Please create a model diagram first"), wxICON_EXCLAMATION);
	}
}
...
void frmDatabaseDesigner::OnNewModel(wxCommandEvent &event)
{
	...
	if(answer != wxCANCEL)
	{
		...
		//Add default unnamed diagram if needed
		if(diagrams->GetPageCount() <= 0)
		{
			diagrams->AddPage(design->createDiagram(diagrams,_("unnamed"),false)->getView(), _("unnamed"));
		}
...
void frmDatabaseDesigner::OnDiagramGeneration(wxCommandEvent &event)
{
	...
	{
		wxMessageBox(_("Warning about generating DDL for a diagram without a diagram"),_("Please create a model diagram first"), wxICON_EXCLAMATION);
	}
}
...
void frmDatabaseDesigner::OnRenameDiagram(wxCommandEvent &event)
{
	hdDrawingView *view = (hdDrawingView *) diagrams->GetPage(diagrams->GetSelection());
	int diagramIndex=view->getIdx();
	wxString name = wxGetTextFromUser(_("Rename diagram: \"")+diagrams->GetPageText(diagramIndex)+_("\""),_("Rename diagram..."),diagrams->GetPageText(diagramIndex),this);
...
void frmDatabaseDesigner::OnChangeConnection(wxCommandEvent &event)
{
	...
	//666	sqlResult->SetConnection(connection);
	//666	title = wxT("Query - ") + cbConnection->GetValue();
		setExtendedTitle();

/*666		//Refresh GQB Tree if used
		if(connection && !firstTime)
		{
			controller->getTablesBrowser()->refreshTables(connection);
...
void frmDatabaseDesigner::OnSelectSchema(wxCommandEvent& WXUNUSED(event))
{
	wxMessageBox(_("nah na na na na"));
	selectedSchema->SetValue(_("nah na na na na"));
	ddSelectSchemaDialog *selectSchema = new ddSelectSchemaDialog(_("Select an Scheme to work with model"),_("Select an Scheme to work with model"),this);
	selectSchema->ShowModal();
}
