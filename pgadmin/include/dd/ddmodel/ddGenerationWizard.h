...
public:
	...
	wxWizardPage *GetFirstPage() const { return initialPage; }
	ddDatabaseDesign* getDesign() {return figuresDesign;};	
...
public:
	...
	int countSelTables() {return m_selTables->GetCount();};
	wxString getSelTableName(int index){return m_selTables->GetString(index);};
private:
...
public:
    ...
    void SetNext(wxWizardPage *next) { m_next = next; }
	void populateGrid();
	wxDDGrid* getGrid() {return reportGrid;};
private:
	void OnWizardPageChanging(wxWizardEvent& event);
